...
class SeriesController extends MainModeratorController
{
    ...
    public function series_list()    { 
        ...
        {
            $keyword = $_GET['s'];  
            $series_list = Series::where("moderator_id",$moderatorId)->where("series_name", "LIKE","%$keyword%")->orderBy('series_name')->paginate(10);
        ...
        else if(isset($_GET['language_id']))
        {
            ...
            $series_list = Series::where("moderator_id",$moderatorId)->where("series_lang_id", "=",$language_id)->orderBy('id','DESC')->paginate(10);

            $series_list->appends(\Request::only('language_id'))->links();
        }
        else
        {
            $series_list = Series::where("moderator_id",$moderatorId)->orderBy('id','DESC')->paginate(10);

        }
        return view('moderator.pages.series_list',compact('page_title','series_list','language_list'));
    }
    ...
    public function addSeries() { 
        ...
        return view('moderator.pages.addeditseries',compact('page_title','language_list','genre_list'));
    }
    ...
    { 
        ...
        $series_obj->status = '0';//$inputs['status']; 
        ...
        $series_obj->moderator_id = $moderatorId;
    ...
    {       
        ...
        $language_list = Language::orderBy('language_name')->get();
        $genre_list = Genres::orderBy('genre_name')->get();   
        return view('moderator.pages.addeditseries',compact('page_title','series_info','language_list','genre_list'));
    }	 
