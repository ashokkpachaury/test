...
class MoviesController extends MainModeratorController
{
    ...
    { 
        ...
        {
            $keyword = $_GET['s'];  
            $movies_list = Movies::where("moderator_id",$moderatorId)->where("video_title", "LIKE","%$keyword%")->orderBy('video_title')->paginate(10);
            $movies_list->appends(\Request::only('s'))->links();
        }    
        else if(isset($_GET['language_id']))
        {
            $language_id = $_GET['language_id'];
            $movies_list = Movies::where("moderator_id",$moderatorId)->where("movie_lang_id", "=",$language_id)->orderBy('id','DESC')->paginate(10);
            $movies_list->appends(\Request::only('language_id'))->links();
        }
        else
        {
            $movies_list = Movies::where("moderator_id",$moderatorId)->orderBy('id','DESC')->paginate(10);
        } 
         
        return view('moderator.pages.movies_list',compact('page_title','movies_list','language_list'));
    }
    ...
    public function addMovie()    { 
        ...
        return view('moderator.pages.addeditmovie',compact('page_title','language_list','genre_list'));
    }
    ...
    { 
        ...
        $movie_obj->status = '0';//$inputs['status'];  
        ...
        $movie_obj->moderator_id = $moderatorId;
    ...
    {      
          if(Auth::User()->usertype!="Moderator")
        {
          ...
          return view('moderator.pages.addeditmovie',compact('page_title','movie','language_list','genre_list'));
