...
class LiveTvController extends MainModeratorController
{
    ...
    { 
        ...
        {
            $keyword = $_GET['s'];  
            $live_tv_list = LiveTV::where("moderator_id",$moderatorId)->where("channel_name", "LIKE","%$keyword%")->orderBy('channel_name')->paginate(10);
        ...
        {
            $cat_id = $_GET['cat_id'];
            $live_tv_list = LiveTV::where("moderator_id",$moderatorId)->where("channel_cat_id", "=",$cat_id)->orderBy('id','DESC')->paginate(10);

            $live_tv_list->appends(\Request::only('cat_id'))->links();
        }
        else
        {
            $live_tv_list = LiveTV::where("moderator_id",$moderatorId)->orderBy('id','DESC')->paginate(10);
        } 
         
        return view('moderator.pages.live_tv_list',compact('page_title','live_tv_list','cat_list'));
    }
    ...
    public function addTv()    { 
        ...
        return view('moderator.pages.addeditlivetv',compact('page_title','cat_list'));
    }
    ...
    { 
        ...
        if(!empty($inputs['id'])){
         ...
         $tv_obj->moderator_id = $moderatorId;
         $tv_obj->save();
    ...
    {      
          ...
          return view('moderator.pages.addeditlivetv',compact('page_title','tv_info','cat_list'));
