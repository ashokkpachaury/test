    ...
    public function season_list()    { 
        ...
        if(isset($_GET['s']))
        {
            ...
            $season_list = DB::table('season')
                           ...
                           ->selectRaw("season.*,CASE WHEN SUBSTRING(season.season_poster,1,4) = 'http' THEN season.season_poster ELSE CONCAT('".$asset_url."',season.season_poster) END as season_poster,series.series_name")
        ...
        else
        {
            ...
            //$season_list = Season::with orderBy('id','DESC')->paginate(10);
            $asset_url = asset("upload/source")."/";
            $season_list = DB::table('season')
                           ->join('series', 'season.series_id', '=', 'series.id')
                           ->selectRaw("season.*,CASE WHEN SUBSTRING(season.season_poster,1,4) = 'http' THEN season.season_poster ELSE CONCAT('".$asset_url."',season.season_poster) END as season_poster,series.series_name")
